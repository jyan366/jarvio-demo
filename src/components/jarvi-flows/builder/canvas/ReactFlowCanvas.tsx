...
import { ReactFlow, MiniMap, Controls, Background, useNodesState, useEdgesState, addEdge, Node, Edge, Connection, NodeTypes, EdgeTypes, MarkerType, Position } from '@xyflow/react';
...
export function ReactFlowCanvas({
  ...
  const convertToNodes = useCallback((): Node[] => {
      ...
      return {
        ...
        data: {
          ...
          onBlockUpdate: (updates: Partial<FlowBlock>) => {
            if (block) {
              const updatedBlocks = blocks.map(b => b.id === block.id ? {
                ...b,
                ...updates
              } : b);
              onBlocksChange(updatedBlocks);
          ...
          onDelete: () => {
            const updatedSteps = steps.filter(s => s.id !== step.id).map((s, idx) => ({
              ...s,
              order: idx
            }));
          ...
          onAttachBlock: () => {
            ...
            onStepsChange(steps.map(s => s.id === step.id ? {
              ...s,
              stepType: 'block',
              isAgentStep: false
            } : s));
    ...
    if (steps.length > 0) {
      const lastStepPosition = steps[steps.length - 1].canvasPosition || {
        x: 400 + (steps.length - 1) * 450,
        y: 100
      };
      const nextStepNode = {
        id: 'next-step',
    ...
    console.log('All nodes created:', nodes.map(n => ({
      id: n.id,
      type: n.type,
      position: n.position
    })));
  ...
  const handleNodeDragStop = useCallback((event: any, node: Node) => {
    const updatedSteps = steps.map(step => step.id === node.id ? {
      ...step,
      canvasPosition: {
        x: node.position.x,
        y: node.position.y
  ...
  const handleAutoArrange = () => {
    const updatedSteps = steps.map((step, index) => ({
      ...
      canvasPosition: {
        x: 400 + index * 450,
        // Start further from trigger, increased spacing
        y: 100
      }
    }));
    onStepsChange(updatedSteps);
  ...
  return <div className="w-full h-full relative bg-background font-inter transition-colors duration-200">
      ...
      <div className="absolute top-6 left-6 z-10">
        <Button onClick={() => setAddPanelOpen(true)} className="gap-2 bg-card hover:bg-accent border border-border text-foreground font-medium px-4 py-2 rounded-lg transition-all duration-200 shadow-lg">
      ...
      <div className="absolute bottom-6 right-6 z-10 flex items-center gap-3">
        <Button onClick={onStartFlow} disabled={isRunningFlow || steps.length === 0} className="gap-2 bg-green-600 hover:bg-green-700 text-white font-medium px-4 py-2 rounded-lg transition-all duration-200 shadow-lg">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button disabled={isTestRunning || steps.length === 0} className="gap-2 bg-blue-600 hover:bg-blue-700 text-white font-medium px-4 py-2 rounded-lg transition-all duration-200 shadow-lg">
              <TestTube className="w-4 h-4" />
              {isTestRunning ? "Testing..." : "Test Flow"}
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="bg-popover border border-border text-popover-foreground rounded-lg shadow-lg z-50">
            <DropdownMenuItem onClick={() => simulateFlowExecution(false)} className="hover:bg-accent focus:bg-accent">
              <Play className="w-4 h-4 mr-2 text-green-500" />
              Test Success
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => simulateFlowExecution(true)} className="hover:bg-accent focus:bg-accent">
      ...
      <AddStepPanel onAddStep={handleAddStep} isOpen={addPanelOpen} onClose={() => setAddPanelOpen(false)} availableBlockOptions={availableBlockOptions} />
      
      <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect} onNodeDragStop={handleNodeDragStop} nodeTypes={nodeTypes} edgeTypes={edgeTypes} fitView snapToGrid snapGrid={[20, 20]} defaultViewport={{
      x: 0,
      y: 0,
      zoom: 1
    }} minZoom={0.2} maxZoom={2} className="bg-transparent">
        <Background color="hsl(var(--border))" gap={40} size={0.5} className="opacity-30 dark:opacity-20" />
        <Controls showInteractive={false} className="bg-card border border-border rounded-lg shadow-lg [&>button]:bg-card [&>button]:border-border [&>button]:text-foreground [&>button:hover]:bg-accent absolute bottom-20 left-6" />
        <MiniMap position="bottom-left" nodeStrokeColor="hsl(var(--primary))" nodeColor={node => {
      ...
      }} maskColor="hsla(var(--background), 0.8)" pannable zoomable className="bg-card border border-border rounded-lg shadow-lg !absolute !bottom-6 !left-6" />
      </ReactFlow>

      <BlockConfigDialog block={selectedBlock} isOpen={showBlockConfig} onClose={() => {
      setShowBlockConfig(false);
      setSelectedBlock(null);
    }} />

      <AttachBlockDialog isOpen={attachBlockDialogOpen} onClose={() => {
