...
export function MyBlocksSection() {
  ...
  const categories: Category[] = [{
    id: 'collect',
    name: 'Collect',
    description: 'Retrieves data from Amazon, scraped sites, or files'
  }, {
    id: 'think',
    name: 'Think',
    description: 'Interprets data using AI or logic'
  }, {
    id: 'act',
    name: 'Act',
    description: 'Performs an action like messaging, updating listings, or creating tasks'
  }];
  ...
  const getFilteredBlocks = () => {
    ...
    return categoryBlocks.filter(block => block.name.toLowerCase().includes(searchTerm.toLowerCase()) || block.summary.toLowerCase().includes(searchTerm.toLowerCase()) || block.description.toLowerCase().includes(searchTerm.toLowerCase()));
  };
  ...
  return <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          <CategoryFilters categories={categories.map(cat => ({
          ...cat,
          name: `${cat.name} (${blockCounts[cat.id as keyof typeof blockCounts]})`
        }))} selectedCategory={selectedCategory} onCategorySelect={setSelectedCategory} getCategoryColor={getCategoryColor} />
        ...
        <div className="max-w-md">
          <Input placeholder={`Search ${selectedCategory} blocks...`} className="w-full" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>
      ...
      {/* Blocks grid */}
      <BlocksGrid blocks={filteredBlocks} selectedCategory={selectedCategory} getCategoryColor={getCategoryColor} onBlockClick={handleBlockClick} />

      {/* Block Detail Modal */}
      <BlockDetailModal isOpen={isModalOpen} onOpenChange={setIsModalOpen} selectedBlock={selectedBlock} connectedServices={connectedServices} onServiceConnection={handleServiceConnection} onActivateBlock={handleActivateBlock} />
    </div>;
}