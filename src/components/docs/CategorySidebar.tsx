...
export function CategorySidebar({
  ...
  const categories: CategoryInfo[] = [{
    ...
    // Always show Flow Activity and Recent
    if (category.id === 'activity' || category.id === 'recent') return true;
  ...
  return <div className="flex flex-col h-full">
      ...
      <ScrollArea className="flex-1 scrollbar-hide">
        <div className="p-2 space-y-1">
          ...
          return <Button key={category.id} variant="ghost" size="sm" onClick={() => onCategoryClick(category.id)} className={cn("w-full justify-start h-auto p-3 text-left", isActive && "bg-primary/10 text-primary font-medium")}>
                <div className="flex items-start gap-3 w-full">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={cn("text-xs px-1.5 py-0.5 rounded-full bg-muted", isActive && "bg-primary/20 text-primary")}>
                        {category.count}
                      </span>
                    </div>
