...
import { fetchTasks, fetchSubtasks, createTask, createSubtasks, addSubtask, deleteSubtask, toggleSubtask, SupabaseTask, SupabaseSubtask } from "@/lib/supabaseTasks";
...
export default function TaskWork() {
  ...
  const handleGenerateSteps = async () => {
    ...
    try {
      ...
      const createdSteps = await createSubtasks(steps.map(s => ({
      ...
      setTaskState(prev => {
        if (!prev) return prev;
        const withNew = [...prev.subtasks, ...createdSteps.map((s, i) => {
          return {
            id: s.id,
            title: s.title,
  ...
  const handleUpdateSubtask = (field: keyof Subtask, value: any) => {
    ...
    setTaskState(prev => {
      if (!prev) return prev;
      const updatedSubs = prev.subtasks.map((st, i) => i === focusedSubtaskIdx ? {
        ...st,
        [field]: value
      } : st);
      return {
  ...
  return <MainLayout>
      <div className="w-full h-[calc(100vh-4rem)] max-w-screen-2xl mx-auto flex flex-col md:flex-row gap-0 items-stretch bg-background overflow-hidden">
        <main className="flex-1 min-w-0 p-1 sm:p-2 md:p-6 lg:p-10 bg-white border-r-[1.5px] border-[#F4F4F8] flex flex-col overflow-y-auto px-[15px]">
